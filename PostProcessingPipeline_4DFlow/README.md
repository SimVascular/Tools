...
1. **(Optional)** Extract .tar/.zip file downloaded from Arterys which contains all of the images (.dcm dicom files) for the entire scan, which may include the 4DMRI as well as standard MR scans.
    ...
    2. To add the path to the .tar/.zip file, you can drag and drop the file into the terminal. If you want to extract multiple files, you can drag and drop multiple .tar files into the terminal.

2. **Convert Arterys 4DMRI scan to readable and manipulated .vtk files.** Extract .tar/.zip of dicom images from Arterys of the 4DMRI scan to vti images sorted by magnitude. Map magnitude images onto velocity data from numpy array.
...
```
            imageFile: path to compressed folder of dicom files or parent folder where IMAGE/ has already been extracted (by imagesorting.py)
            velFile: path to velocity numpy array from Arterys
            saveFileName: file name prefix to save outputs with (e.g PREFIX\_##.vtk)
            imageExtracted: True or False if compressed folder already extracted
            python arterys\_mapping.py /arterys\_mapping.py /home/example/dicom\_4dmri.zip /home/example/velocity.npy PREFIX False
```
        2. Output: (if extracting DICOM files) IMAGE/ folder with all dicom images sorted into magnitude dicoms by cardiac frame, MR\_headerinfo.txt containing metadata information about venc, heart rate, etc.; PREFIX\_##.vtk saved to parent directory of IMAGE/
3. Create 3D models of the initial timepoint and the max deformed vessel timepoint using SimVascular
...
4. Use Deformetrica&#39;s shape analysis registration algorithm to interpolate the 3D shape of the PAs in between the initial and max deformed state as well as from the max deformed state back to diastolic state. These two registrations represent systolic upstroke and the gradual systolic downstroke.
...
```
        do_registration = True (set all other "do_<>" options and the centerline_already_extracted to False)
          (If you will be doing Step 6 in conjunction with this step (recommended), set the do_volumeremeshing to True)
        data_path: set to the directory that contains the solid model surface mesh files (i.e mag1.vtp created in Step 4)
        subject_id: set to a prefixed used for all outputted files from Deformetrica
        dia_tmpl: set to the .vtp surface mesh model for the initial timepoint
        sys_tmpl: set to the .vtp surface mesh model for the maximum deformed timepoint
        total_timepoints: total number of timepoints in the cardiac cycle (i.e total number of vti_imgs)
```
        2. To call this script, go to the environment that contains vmtk and type in the following command
```
        python <path to postprocessing_4dmri.py> <path to the input.txt file>
```

5. Remesh the output of Deformetrica&#39;s registration output into volume meshes
    ...
    2. Check that these interpolated models match the image. Load the remeshed models and the vti\_img files into Paraview and visually check how well it matches. You can change the opacity and take slices to check.
6. [Create centerlines](#_Instructions_for_extracting) for each remeshed model outputted in step 6.
...
7. Resample the 4DMRI vtk files with the remeshed registration outputs at each timepoint to constrain the velocity data to only the MPA.
8. Extract slices from the 4DMRI resampled PA files and compute quantities of interest for each
    1. Additional helper functions and analysis can be added to this part of the code to do more complex analysis

\*For steps 7-8, use the postprocessing\_4dmri.py script and a modified input.txt file
...
1. Import the surface mesh of the geometries outputted from deformetrica (labelled as \&lt;patientname\&gt;\_\_\&lt;timepoint\&gt;\_surfacemesh.vtk). Do this for each timepoint in the scan (i.e all the surface meshes outputted by deformetrica)
    ...
    5. Once the cap is highlighted and turns yellow, there should be a face in the Face List that is highlighted. That corresponds to the face that you&#39;ve selected on the model. Relabel that face name to the corresponding cap (label these exactly MPA, LPA, or RPA in all caps).
    6. When you have finished labelling all of the caps, select those faces in the Modeling Tab, right click and select _Change Type_. Change the type to cap and press OK.
    7. Sort the Face List by **Name**. Select all of the noname\_# faces from the list. In the Face Ops on the right, select Combine. Relabel the combined name to wall and change the cap type to wall.
    8. Save

2. Run the simvascular centerline extraction python script (sv\_centerline\_extraction.py located in the ArterysDataProcessing repo).
...
```
[directory to simvascular application] --python -- sv_automated_cap_cl.py --path-name=[path to model's geometry shape_analysis/registration_output] --prefix-name=[prefix naming] --timepoints=[number of timepoints] --clip-distance=0.1 --clip-width-scale=4 --mesh-scale=0.3 --remesh-scale=0.25
```